...
EstoqueStatus estoque_cadastrar_produto(Estoque *estoque, const char16_t *codbar, const char16_t *nome, double preco, uint32_t quantidade, const char16_t *categoria, uint32_t *id_gerado);

/*
 * Insere uma cópia de produto já existente.
 * Útil durante o carregamento de dados persistidos.
 */
EstoqueStatus estoque_inserir_produto(Estoque *estoque, const Produto *produto);

EstoqueStatus estoque_atualizar_produto(Estoque *estoque, const char16_t *codbar, const char16_t *nome, double preco, uint32_t quantidade, const char16_t *categoria);

/*
 * Alterar o código exige operação própria porque o vetor por_codigo
 * precisa ser reposicionado para continuar ordenado.
 */
EstoqueStatus estoque_alterar_codigo(Estoque *estoque, const char16_t *codigo_atual, const char16_t *novo_codigo);

EstoqueStatus estoque_remover_produto(Estoque *estoque, const char16_t *codbar);

Produto *estoque_buscar_por_codigo(Estoque *estoque, const char16_t *codbar);
const Produto *estoque_buscar_por_codigo_const(const Estoque *estoque, const char16_t *codbar);

/*
 * Retorna o total de correspondências encontradas.
 * Quando saida não é NULL, grava no máximo capacidade_saida ponteiros.
 */
size_t estoque_buscar_por_nome(const Estoque *estoque, const char16_t *trecho_nome, const Produto **saida, size_t capacidade_saida);

size_t estoque_listar_por_categoria(const Estoque *estoque, const char16_t *categoria, const Produto **saida, size_t capacidade_saida);

size_t estoque_listar_baixo_estoque(const Estoque *estoque, uint32_t limite, const Produto **saida, size_t capacidade_saida);
...
EstoqueStatus estoque_registrar_venda(Estoque *estoque, const char16_t *codbar, uint32_t quantidade, Transacao *transacao_gerada);
 ...
 */
EstoqueStatus estoque_inserir_transacao(Estoque *estoque, const Transacao *transacao);
 ...
 */
EstoqueStatus estoque_restaurar_controle(Estoque *estoque, uint32_t proximo_produto_id, uint64_t proxima_transacao_id);

size_t estoque_total_produtos(const Estoque *estoque);
const Produto *estoque_produto_ordenado(const Estoque *estoque, size_t indice);
const Produto *estoque_produto_cadastrado(const Estoque *estoque, size_t indice);
